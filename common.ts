...
import { readableStreamToString, PingIdentityDirectoryError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PingIdentityDirectoryError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PingIdentityDirectoryError(e, e.response?.data)
                }
                if (innerError instanceof PingIdentityDirectoryError) {
                    // Got "PingIdentityDirectoryError" from the above try block
                    throw innerError;
                }
